...
const UserDashboard = () => {
  const { currentUser, appointments, setAppointments, billings, setBillings, handleLogout } = useContext(AppContext);
      ...
      <div className="main-content">
              ...
              <div className="news-grid">
                {newsItems.map((n, i) => (
                  <div key={i} className="news-item">
                    <img src={n.img} alt={n.title} loading="lazy" style={{ width: '100%', height: '150px', objectFit: 'cover', borderRadius: '16px' }} />
        ...
        {showPaymentModal && selectedBill && (
          <div className="payment-modal-overlay" onClick={() => !paymentProcessing && setShowPaymentModal(false)}>
            <div className="payment-modal" onClick={e => e.stopPropagation()} style={{ background: 'white', borderRadius: '32px', width: '90%', maxWidth: '500px', maxHeight: '85vh', overflowY: 'auto' }}>
              <div className="payment-modal-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '20px 24px', borderBottom: '1px solid #eee' }}>
                <h3>To'lov amalga oshirish</h3>
                <button className="close-modal-btn" onClick={() => !paymentProcessing && setShowPaymentModal(false)} disabled={paymentProcessing} style={{ background: 'none', border: 'none', cursor: 'pointer' }}>
                  <AiOutlineClose size={20} />
                </button>
              </div>
              
              <div className="payment-modal-body" style={{ padding: '24px' }}>
                <div className="payment-bill-info" style={{ background: '#f8f9fa', borderRadius: '20px', padding: '16px', marginBottom: '20px' }}>
                  <h4 style={{ margin: '0 0 12px 0' }}>{selectedBill.patientName}</h4>
                  <div className="payment-details">
                    <div className="payment-detail-item" style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                      <span>Umumiy summa:</span>
                      <strong>{selectedBill.total.toLocaleString()} UZS</strong>
                    </div>
                    <div className="payment-detail-item" style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                      <span>To'langan:</span>
                      <strong style={{color: '#10b981'}}>{(selectedBill.paid || 0).toLocaleString()} UZS</strong>
                    </div>
                    <div className="payment-detail-item" style={{ display: 'flex', justifyContent: 'space-between' }}>
                      <span>Qoldiq:</span>
                      <strong style={{color: '#ef4444'}}>{calculateRemainingAmount(selectedBill).toLocaleString()} UZS</strong>
                    </div>
                  </div>
                </div>
                
                <div className="payment-amount-input" style={{ marginBottom: '24px' }}>
                  <label style={{ display: 'block', marginBottom: '8px', fontWeight: 500 }}>To'lov summasi (UZS)</label>
                  <input
                    type="number"
                    value={paymentAmount}
                  ...
                  <div className="amount-suggestions" style={{ display: 'flex', gap: '8px', marginTop: '8px' }}>
                    <button onClick={() => setPaymentAmount(Math.ceil(calculateRemainingAmount(selectedBill) * 0.25))} style={{ padding: '6px 12px', borderRadius: '20px', border: '1px solid #ddd', background: 'white', cursor: 'pointer' }}>25%</button>
                    <button onClick={() => setPaymentAmount(Math.ceil(calculateRemainingAmount(selectedBill) * 0.5))} style={{ padding: '6px 12px', borderRadius: '20px', border: '1px solid #ddd', background: 'white', cursor: 'pointer' }}>50%</button>
                    <button onClick={() => setPaymentAmount(Math.ceil(calculateRemainingAmount(selectedBill) * 0.75))} style={{ padding: '6px 12px', borderRadius: '20px', border: '1px solid #ddd', background: 'white', cursor: 'pointer' }}>75%</button>
                    <button onClick={() => setPaymentAmount(calculateRemainingAmount(selectedBill))} style={{ padding: '6px 12px', borderRadius: '20px', border: '1px solid #ddd', background: 'white', cursor: 'pointer' }}>100%</button>
                  </div>
                </div>
                
                <div className="payment-methods">
                  <h4 style={{ margin: '0 0 12px 0' }}>To'lov usulini tanlang</h4>
                  <div className="method-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(100px, 1fr))', gap: '12px' }}>
                    {PAYMENT_METHODS.map(method => (
                      ...
                      >
                        <span className="method-icon" style={{ fontSize: '24px', display: 'block' }}>{method.icon}</span>
                        <span style={{ fontSize: '12px' }}>{method.name}</span>
                      </button>
                    ))}
                  </div>
                </div>
              ...
              <div className="payment-modal-footer" style={{ padding: '20px 24px', borderTop: '1px solid #eee' }}>
                ...
                <div className="payment-actions" style={{ display: 'flex', gap: '12px' }}>
                  ...
                  >
                    {paymentProcessing ? (
                      <>
                        <span style={{ display: 'inline-block', width: '16px', height: '16px', border: '2px solid white', borderTopColor: 'transparent', borderRadius: '50%', animation: 'spin 0.6s linear infinite', marginRight: '8px' }}></span>
            ...
            <div className="receipt-modal" onClick={e => e.stopPropagation()} style={{ background: 'white', borderRadius: '32px', width: '90%', maxWidth: '500px', maxHeight: '85vh', overflowY: 'auto' }}>
              ...
              <div className="receipt-modal-body" style={{ padding: '24px' }}>
                ...
                <div className="receipt-services" style={{ marginBottom: '20px' }}>
                  <h4 style={{ margin: '0 0 12px 0' }}>Xizmatlar:</h4>
                  {selectedReceipt.services.map((service, index) => (
                    <div key={index} className="service-row" style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0', borderBottom: '1px solid #eee' }}>
                ...
                {notifications.length > 0 ? (
                  notifications.map(notification => (
                    <div 
                      ...
                      onClick={() => handleNotificationAction(notification)}
                      style={{ display: 'flex', gap: '14px', padding: '16px', borderBottom: '1px solid #eee', cursor: 'pointer' }}
                    >
