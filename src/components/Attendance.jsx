        ...
        {activeTab === 'checkin' && (
          <div className="checkin-section">
            ...
            <div className="current-action">
              <h2>{scanMode === 'checkin' ? 'Ishga Kirish' : 'Ishdan Chiqish'}</h2>
            ...
            <div className="login-options">
              <div className={`login-card ${loginMethod === 'qr' ? 'active' : ''}`}>
                ...
                <button className="action-btn primary" onClick={() => { setLoginMethod('qr'); handleQRScan(); }}>
                  <FaQrcode /> QR Skanerlash
                </button>
              </div>

              <div className={`login-card ${loginMethod === 'pin' ? 'active' : ''}`}>
                <div className="card-icon"><FaKey /></div>
                <h3>PIN Kod</h3>
                ...
                <div className="keypad">
                  {pinButtons.map((btn, idx) => (
                    <button key={idx} className={`keypad-btn ${btn === 'C' || btn === '⌫' ? 'special' : ''}`} onClick={() => {
                ...
                <button className="action-btn secondary" onClick={handlePinLogin} disabled={pinInput.length !== 4}>
        ...
        {activeTab === 'employees' && (
          <div className="employees-section">
            <div className="section-header">
              ...
              <button className="add-btn" onClick={addEmployee}><FaUserPlus /> Yangi Xodim</button>
            </div>

            <div className="search-box">
              <FaSearch />
              <input type="text" placeholder="Xodim qidirish..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
            </div>

            <div className="employees-grid">
              {employees.filter(emp => emp.name.toLowerCase().includes(searchTerm.toLowerCase()) || emp.position.toLowerCase().includes(searchTerm.toLowerCase()) || emp.pin.includes(searchTerm)).map(emp => {
                  ...
                  <div key={emp.id} className="employee-card">
                    <div className="card-header">
                      <div className="employee-photo">
                        {emp.photo ? <img src={emp.photo} alt={emp.name} /> : <div className="photo-placeholder"><FaUser /></div>}
                        <button className="photo-btn" onClick={() => uploadEmployeePhoto(emp.id)} title="Rasm yuklash"><FaCamera /></button>
                      </div>
                      <div className="employee-info">
                        <h3>{emp.name}</h3>
                        <p className="position">{emp.position}</p>
                        <div className="employee-pin"><FaIdCard /> PIN: {emp.pin}</div>
                      </div>
                    </div>
                    <div className="card-body">
                      <div className="stats-row">
                        <div className="stat"><span className="stat-value">{empRecords.length}</span><span className="stat-label">Kun</span></div>
                        <div className="stat"><span className="stat-value">{todayRecord ? (todayRecord.checkOut ? '✅' : '🔄') : '❌'}</span><span className="stat-label">Bugun</span></div>
                        <div className="stat"><span className="stat-value">{emp.joinDate}</span><span className="stat-label">Qo'shilgan</span></div>
                      </div>
                    </div>
                    <div className="card-footer">
                      <button className="action-btn small" onClick={() => { setActiveTab('checkin'); setScanMode('checkin'); setPhotoFor({ type: 'checkin', employeeId: emp.id, method: 'manual' }); setShowCamera(true); startCamera(); }}>
                        <FaSignInAlt /> Kirish
                      </button>
                      <button className="action-btn small" onClick={() => { setActiveTab('checkin'); setScanMode('checkout'); setPhotoFor({ type: 'checkout', employeeId: emp.id, method: 'manual' }); setShowCamera(true); startCamera(); }}>
          ...
          <div className="history-section">
            <div className="section-header">
              ...
              <div className="filters">
                ...
                <button className="clear-btn" onClick={() => { setFilterDate(""); setFilterStatus("all"); }}><FaTimes /> Tozalash</button>
              </div>
            </div>
            ...
            <div className="records-table">
              <table className="attendance-table">
                <thead>
                  <tr><th>Xodim</th><th>Sana</th><th>Kirish</th><th>Chiqish</th><th>Holat</th><th>Rasm</th></tr>
                </thead>
                <tbody>
                  {records.filter(rec => (!filterDate || rec.date === filterDate) && (filterStatus === 'all' || rec.status === filterStatus)).slice(0, 20).map(rec => {
                    const emp = employees.find(e => e.id === rec.employeeId);
                    if (!emp) return null;
                    const status = getStatusInfo(rec.status);
                    return (
                      <tr key={rec.id}>
                        <td className="employee-cell"><div className="avatar">{emp.photo ? <img src={emp.photo} alt={emp.name} /> : <FaUser />}</div><div><strong>{emp.name}</strong><small>{emp.position}</small></div></td>
                        <td>{rec.date}</td><td><span className={`time ${rec.checkIn > "08:00" ? 'late' : ''}`}>{rec.checkIn}</span></td>
                        <td><span className="time">{rec.checkOut || "-"}</span></td>
                        <td><span className="status-badge" style={{ background: status.bg, color: status.color }}>{status.icon} {status.text}</span></td>
                        <td>{rec.photo ? <button className="view-photo-btn" onClick={() => { setCapturedPhoto(rec.photo); setShowPhotoModal(true); }}><FaImage /> Ko'rish</button> : <span className="no-photo">-</span>}</td>
                      </tr>
                    );
                  })}
                </tbody>
        ...
        {activeTab === 'reports' && (
          <div className="reports-section">
            ...
            <div className="stats-cards">
              ...
              <div className="stat-card"><div className="stat-icon absent"><FaTimes /></div><div className="stat-content"><h3>{absentCount}</h3><p>Kelmagan</p></div></div>
              <div className="stat-card"><div className="stat-icon total"><FaDatabase /></div><div className="stat-content"><h3>{records.length}</h3><p>Jami Yozuvlar</p></div></div>
            </div>
            <div className="reports-grid">
              <div className="report-card"><h3>Kunlik Faollik</h3><div className="report-content"><p><strong>Bugun:</strong> {today}</p><p><strong>Jami kelgan:</strong> {presentCount} ta</p><p><strong>Kechikkanlar:</strong> {lateCount} ta</p><p><strong>Kelmaganlar:</strong> {absentCount} ta</p></div></div>
              <div className="report-card"><h3>Eng Faol Xodimlar</h3><div className="report-content">{employees.map(emp => ({ ...emp, count: records.filter(r => r.employeeId === emp.id).length })).sort((a, b) => b.count - a.count).slice(0, 3).map((emp, idx) => (<div key={emp.id} className="top-employee"><span className="rank">{idx + 1}.</span><span className="name">{emp.name}</span><span className="count">{emp.count} kun</span></div>))}</div></div>
            </div>
          </div>
        )}
      </main>
